...
player = {x=64, y=120, spr=0}        -- カ▒オよカ█オぬオみカ🐱 オまオはカ█オゆオむオぬ: id 0
bullets = {}
enemies = {}
score = 0
shoot_cooldown = 0
...
function _update()
  ...
  elseif game_state == "play" then
    ...
    if btn(4) and shoot_cooldown > 15 then
      add(bullets, {x=player.x+3, y=player.y, spr=2})  -- オよカ⬇️オめカ◆: カ▒オよカ█オぬオみカ🐱 2 (オもオゆオへオやオゆ カ▒オゆオほオひオぬカ🐱カ😐)
      shoot_cooldown = 0
    end
    ...
    if rnd(100) < 1.5 then
      add(enemies, {x=rnd(120), y=-8, spr=1})  -- オのカ█オぬオは: カ▒オよカ█オぬオみカ🐱 id 1
    end
