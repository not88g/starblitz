...
-- オかオˇオきオˇオうオˇオえオえオつオˇ オ▤ オえオ…オくオけオきオおオ▥オあオ▤
...
-- カ▒オゆカ▒カ🐱オゆカ◆オやオまカ◆: logo ヌ●★ play ヌ●★ game_over ヌ●★ win
game_state = "logo"
logo_timer = 0
logo_duration = 120     -- 2 カ▒オふオむカ⬇️オやオひカ⬅️ オよカ█オま 60 fps

-- オよカ█オゆオのオふカ█オむオぬ オよオふカ█オふカ▒オふカ♥オふオやオまオみ オよカ█カ◆オもオゆカ⬇️オはオゆオめカ😐オやオまオむオゆオの
...
function _update()
  ...
  elseif game_state == "play" then
    ...
    -- オゆオねオやオゆオのオめオふオやオまオふ オのカ█オぬオはオゆオの オま カ▒カ🐱オゆオめオむオやオゆオのオふオやオまカ◆
    ...
    for e in all(enemies) do
      ...
      for b in all(bullets) do
        if rect_overlap(b.x, b.y, 2,4, e.x, e.y, 8*e.size, 8*e.size) then
          ...
          del(bullets, b)
          score += 1
          -- オひオゆオよオゆオめオやオまカ🐱オふオめカ😐オやオぬカ◆ オへオまオほオやカ😐 オむオぬオへオひカ⬅️オふ 15 オゆカ♥オむオゆオの
      ...
      if rect_overlap(player.x, player.y, player.w, player.h,
        ...
        lives -= 1
        del(enemies, e)
        if lives <= 0 then game_state = "game_over" end
      end
    end

    -- オよカ█オゆオのオふカ█オむオぬ オよオゆオねオふオひカ⬅️
    if score >= 9999 then
      game_state = "win"
    end
  ...
  elseif game_state == "game_over" or game_state == "win" then
...
function _draw()
  if game_state == "logo" then
    cls(0)
    -- オめオゆオはオゆ オま カ▒オやオふオへオまオやオむオぬ
    local title = "starblitz"
    local tx = flr((128-#title*4)/2)
    local ty = 48
    print(title, tx, ty, 7)
    -- カ▒オやオふオへオまオやオむオぬ カ▒オめオふオのオぬ
    print("*", tx-6, ty, 7)
    -- オやオまオへオやオまオみ カ🐱オふオむカ▒カ🐱
    local info = "stable - v1.0 | made by 88g"
  ...
  elseif game_state == "win" then
    ...
    local my = 48
    print(msg, mx, my, 11)
    local thanks = "thanks for playing our game -88g"
    local tx2 = flr((128-#thanks*4)/2)
    print(thanks, tx2, my+16, 6)
  end
end
